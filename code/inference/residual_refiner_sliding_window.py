"""Full-volume sliding-window inference for U-Net residual refiners."""
...
def sliding_window_refiner_correct(
    v_corrupted: torch.Tensor,
    initializer: ResidualUNet3D,
    refiner: ResidualRefinerUNet3D,
    ...
    """Run frozen initializer followed by residual refiner on a full volume."""
    ...
    log.info(
        "[refiner sliding] shape=%s roi=%s overlap=%.2f mode=%s",
        spatial,
        roi,
        overlap,
        blend_mode,
    ...
    def predictor(patches: torch.Tensor) -> torch.Tensor:
        return refiner(patches.to(sw_dev, non_blocking=True)).float()

    corrected = inferer(condition.float(), predictor).float()
    if return_initial:
        return corrected, initial.float()
    return corrected
