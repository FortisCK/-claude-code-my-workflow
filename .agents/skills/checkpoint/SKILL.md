...
✓ Checkpoint saved: quality_reports/checkpoints/YYYY-MM-DD_<slug>.md
  ...
  Resume command: Codex --continue   (or paste the file's "Resume prompt" into a fresh session)
```
...
- `.Codex/hooks/pre-compact.py` — when `CLAUDE_PRECOMPACT_BLOCK_ON_DRAFT=1` is set, the PreCompact hook will block compaction once per DRAFT plan. `/checkpoint` is the right thing to run when that block fires.
...
**Result:** Next session: `Codex --continue`, then `read quality_reports/checkpoints/2026-04-27_v180-polisci.md and start at action 1`.
