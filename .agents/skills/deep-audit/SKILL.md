...
Focus: **all** executable code in the repo — `.Codex/hooks/*.py`, `.Codex/hooks/*.sh`, `scripts/*.py`, `scripts/*.sh`, `.Codex/scripts/*.sh`. Not just `.Codex/hooks/` — when PR #93 added new code under `scripts/`, the original narrow scope meant Copilot + Codex caught 5 bugs the audit missed.

Hook-specific checks (Stop/PreToolUse/SessionStart protocols, `CLAUDE_PROJECT_DIR` usage, hash-length consistency) apply only to `.Codex/hooks/`. Everything below applies to ALL executable code:
