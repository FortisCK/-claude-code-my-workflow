...
3. 在 150 个有标签和 620 个无标签 CT 体上进行综合评估，包括逐关键点准确性、消融分析、六项具有临床意义的下游 TAVI 规划测量，以及同时涵盖关键点与测量级比较的同测试集完整 10 点多标注者分析。
...
原始测试标注记为标注者 A1。为将模型表现置于人工标注变异的背景中，同一 30 例测试集由标注者 B 完成了一套独立的完整 10 关键点标注，原标注者也完成了一次重复标注，记为标注者 A2。
...
对于新增的多标注者分析，我们分别报告 $P_0$--$P_7$、全部 10 点和冠脉口 $P_8$--$P_9$ 的 MRE，以及完整点集的 SDR 和离群点数。我们还比较 MSL、三铰链外接圆直径及圆面积、冠脉高度、瓣环平面方向和 DLZ 总钙化量。成对结果包括标注者 B 与标注者 A2，以及 TRUST 与这两套新增标注的比较；TRUST 与标注者 A1 的比较对应前述主实验，因此不在此重复。
...
表 10 比较了标注者 B 的独立标注、标注者 A2 的重复标注和 TRUST。

在全部 10 个关键点上，标注者间差异为 2.587±2.287 mm；TRUST 相对于标注者 B 和标注者 A2 的误差分别为 3.213±2.457 mm 和 2.813±1.892 mm。对应的 SDR$_{4.0}$ 分别为 84.67%、77.33% 和 81.00%。

这一差距在新增冠脉口上更明显：标注者 B 与标注者 A2 在 $P_8$--$P_9$ 上的误差为 1.934±1.499 mm，而 TRUST 相对于标注者 B 和标注者 A2 分别为 3.448±2.696 mm 和 3.191±2.626 mm。

这些结果表明，模型整体表现接近但尚未达到观察到的标注者间一致性，冠脉口是最明确的剩余读者参照精度差距。

**表 10 图注翻译。** 同一 30 例测试集上的单中心多标注者分析。标注者 A1 是主实验使用的原始参考；标注者 A2 是同一标注者的重复标注，标注者 B 是独立标注者，因此不重复报告 TRUST 与 A1 的结果。面板（a）报告 MRE（均值±标准差）、完整 10 点 SDR 以及超过 10 mm 的误差点数。面板（b）报告下游 MAE。平面角度是两个瓣环平面法向量之间的夹角。

（a）关键点定位：

| Comparison | $P_0$--$P_7$ MRE (mm) | 10 点 MRE (mm) | $P_8$--$P_9$ MRE (mm) | SDR$_{2.0}$ (%) | SDR$_{4.0}$ (%) | >10 mm 点数 |
|---|---:|---:|---:|---:|---:|---:|
| Reader B vs. Reader A2 | 2.750±2.420 | 2.587±2.287 | 1.934±1.499 | 48.67 | 84.67 | 5 |
| TRUST vs. Reader B | 3.155±2.396 | 3.213±2.457 | 3.448±2.696 | 32.67 | 77.33 | 7 |
| TRUST vs. Reader A2 | 2.719±1.653 | 2.813±1.892 | 3.191±2.626 | 39.00 | 81.00 | 3 |

（b）下游测量 MAE：

| Comparison | MSL (mm) | $D_{\text{circ}}$ (mm) | 三点圆面积 (mm²) | LCO 高度 (mm) | RCO 高度 (mm) | 平面角度 (°) | DLZ 总量 (mm³) |
|---|---:|---:|---:|---:|---:|---:|---:|
| Reader B vs. Reader A2 | 2.936 | 2.167 | 82.962 | 0.836 | 1.301 | 4.155 | 16.068 |
| TRUST vs. Reader B | 3.099 | 2.299 | 88.554 | 1.724 | 2.407 | 5.800 | 21.817 |
| TRUST vs. Reader A2 | 1.826 | 1.232 | 44.344 | 1.899 | 2.177 | 4.872 | 28.853 |

下游结果提供了互补视角。对于 MSL、$D_{\text{circ}}$ 和圆面积，TRUST 相对于标注者 A2 的 MAE 低于标注者 B 与标注者 A2 之间的 MAE，而 TRUST 相对于标注者 B 的误差与标注者间差异相当或略高。这种不对称性说明结果对所选参考标注较为敏感。

冠脉高度的模型-人工 MAE（LCO 1.724--1.899 mm；RCO 2.177--2.407 mm）持续高于标注者 B 与标注者 A2 之间的差异（分别为 0.836 mm 和 1.301 mm）。瓣环平面方向接近标注者间水平（4.872--5.800° vs. 4.155°），而 DLZ 总量 MAE 从标注者之间的 16.068 mm³ 增加到模型-人工比较中的 21.817--28.853 mm³。
...
完整 10 点多标注者比较为人工标注变异提供了直接参考。TRUST-人工的总体 MRE 接近但高于人工-人工差异，其中冠脉口的相对差距最大。

对于铰链点导出的下游测量，一致性取决于所选择的参考标注；对于冠脉高度、平面方向和 DLZ 总量，模型-人工 MAE 也仍高于人工-人工结果。

### 局限

若干局限需要讨论。

第一，当前测试集及其多标注者数据仍来自单中心。虽然一名独立的第二标注者和原标注者的一次重复标注支持完整 10 点和下游测量比较，但该分析并不构成外部验证或经裁决的共识参考。
...
要对关键点精度和冠脉高度筛查能力给出决定性评估，仍需要在低冠脉高度以及其他冠脉阻塞风险特征富集的队列中，使用经裁决的多标注者参考开展多中心外部验证。
...
同一测试集上的完整 10 点多标注者分析表明，模型表现接近但尚未达到观察到的标注者间一致性，并将冠脉口识别为最大的剩余读者参照精度差距。

主要后续工作包括使用经裁决的多标注者参考开展多中心验证、校准推理期不确定性，以及发展直接瓣环轮廓预测以替代当前三点圆形面积近似。
