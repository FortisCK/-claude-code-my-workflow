...
[LEARN:result] **把 residual diffusion 训到收敛,反而让 oracle headroom 缩小——性能天花板是结构性的(已表征)**。2026-06-15 把 pilot 的 5-epoch residual EDM 训到 80 epoch(收敛,loss 0.244→0.17)。结果:(a) ungated posterior mean 从 61.75→41.79 HU(test5),逼近但**不超过** U-Net 的 40.61(+1.19 HU);(b) **voxel-oracle 上限不升反降**:全局 -3.34→-2.83、heart -5.09→-3.81、boundary -4.90→-4.38;(c) 错配的 v1 gate 套到收敛 cache 上全局增益仍是 -0.075(同 v1)。Why: 监督 L1 U-Net ≈ conditional mean,而 diffusion posterior mean 估计的是同一个量,所以收敛时 posterior mean → U-Net,它提议的残差变小、且与 U-Net 已捕获的部分重叠更多 → gate 可兑现的空间反而更小。How to apply: **不要再为「精度」去重训 gate 或继续加大 diffusion**;这条 refinement formulation 的性能上限已封顶。论文性能口径改为「持平 U-Net + 保守 gate 的小幅一致改善 + 免费的标定不确定性」,并把「为何 diffusion refinement 打不过强监督 U-Net」连同 oracle-headroom 表征写成正面的科学贡献。收敛 checkpoint `diffusion_v2_residual/longrun/epoch_080.pt` 保留——它是第一个 σ_r 有意义的后验,下一步评估它的 UQ calibration(novelty #2)。Run card `2026-06-15_1705_residual-diffusion-v2-longrun.md`。
