...
## CATHACTION Task 2 (2026-06-15)

[LEARN:task2-split] The shipped Task 2 split is **single-video phantom validation** (`valid_phantom.txt` = video_0 only) and `prepare_yolo_splits.py` originally deduped train↔valid by **frame name, not video id** → 5 phantom video_0 frames leaked into train while video_0 was 100% of phantom validation (INV-2 violation). Fixed to **video-level** dedup + hard assert; added `scripts/task2/audit_split_disjointness.py` (run it after regenerating ANY Task 2 split — non-zero exit on overlap) and `tests/test_task2_split_disjointness.py`. Leak magnitude was tiny (5/35,788) so it did **not** materially inflate prior mAP; the real Task 2 caveats are single-video phantom eval + score-mode selection-on-eval + animal-dominated combined headline (~107 frames). Treat pre-2026-06-15 Task 2 mAP as directional, not decisional.
