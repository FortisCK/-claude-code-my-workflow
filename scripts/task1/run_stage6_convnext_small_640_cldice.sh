...
TRAIN_LOG="quality_reports/logs/task1_smp_fpn_convnext_small_640_cldice_stage6.log"
MASTER_LOG="${1:-quality_reports/logs/task1_stage6_convnext_small_640_cldice_master.log}"
EVAL_DIR="outputs/task1/stage6_convnext_small_640_cldice_eval"
EVAL_JSON="$EVAL_DIR/convnext_small_640_cldice_hflip_original_full.json"
EVAL_LOG="quality_reports/logs/task1_smp_fpn_convnext_small_640_cldice_stage6_full_eval.log"
EVAL_MANIFEST="configs/task1/splits/released_eval.csv"
CHECKPOINT="outputs/task1/smp_fpn_convnext_small_640_cldice_stage6/best_checkpoint.pt"

mkdir -p "$(dirname "$MASTER_LOG")" "$EVAL_DIR" /tmp/cathaction-matplotlib

exec >> "$MASTER_LOG" 2>&1

echo "==== CATHACTION Task 1 Stage 6 ConvNeXt-Small 640 clDice ===="
...
/home/mingzhang/miniconda3/bin/conda run --no-capture-output -n cardiac-diffusion \
  python -u scripts/task1/evaluate_tta_ensemble_original_space.py \
    --model convnext_small_640_cldice "$CONFIG" "$CHECKPOINT" 1.0 \
    --tta hflip \
...
path = Path("outputs/task1/stage6_convnext_small_640_cldice_eval/convnext_small_640_cldice_hflip_original_full.json")
