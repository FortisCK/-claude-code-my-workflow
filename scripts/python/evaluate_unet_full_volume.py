#!/usr/bin/env python
"""Full-volume evaluation for supervised residual U-Net baselines."""
...
log = logging.getLogger("evaluate_unet_full_volume")
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Evaluate supervised residual U-Net on full 192^3 volumes.")
    ...
    p.add_argument(
        "--out-dir",
        type=Path,
        default=Path("experiments/runs/unet_v1/eval_full_volume"),
    )
    return p
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.device == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA requested but unavailable; full-volume U-Net eval is GPU-only.")
    ...
    for idx, (split_name, case_id) in enumerate(selected):
        ...
        row[f"{prefix}_delta_mae_norm"] = float(row[f"{prefix}_mae_norm"]) - float(row["corrupted_mae_norm"])
        row[f"{prefix}_min"] = float(pred.min().item())
        row[f"{prefix}_max"] = float(pred.max().item())
        rows.append(row)
        log.info(
            "%s case %s | corr MAE %.5f unet %.5f",
            split_name,
            case_id,
            row["corrupted_mae_norm"],
            row[f"{prefix}_mae_norm"],
        )

        if idx < args.figure_cases:
            save_panel(figures_dir / f"{split_name}_case_{case_id}.png", clean, corrupted, pred)
